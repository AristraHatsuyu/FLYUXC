...
> **重要**：以下数组操作函数采用 **原地修改** 语义，直接修改原数组：
> - `push`、`pop`、`shift`、`unshift`、`reverse`、`sort`
> 
> 以下函数返回 **新数组/新值**，不修改原数组：
> - `slice`、`concat`、`filter`、`map`、`reduce`、`find`

#### push(array, value)
在数组末尾添加元素（原地修改）。返回数组的新长度。
```flyux
arr := [1, 2, 3]
newLen := push(arr, 4)      // newLen = 4, arr = [1, 2, 3, 4]
```
...
#### unshift(array, value)
在数组开头添加元素（原地修改）。返回数组的新长度。
```flyux
arr := [1, 2, 3]
newLen := unshift(arr, 0)   // newLen = 4, arr = [0, 1, 2, 3]
```
...
#### throwErr(errorType, message?)
主动抛出错误。与内置函数错误不同，`throwErr` 总是抛出错误（无需 `!` 后缀）。

**参数**:
- `errorType: str` - 错误类型字符串，可选值：
  - `"error"` - 一般错误（错误代码 1）
  - `"type"` - 类型错误（错误代码 3）
  - `"bounds"` - 越界错误（错误代码 4）
  - `"io"` - IO错误（错误代码 5）
  - `"math"` - 数学错误（错误代码 6）
  - 其他字符串 - 默认为一般错误
- `message: str` - 可选的错误详细消息

**行为**:
- 如果在 `T>` try-catch 块内调用，错误会被 catch 捕获
- 如果不在 try-catch 块内，程序会立即终止并显示错误信息

```flyux
// 示例 1: 在 try-catch 中使用
T> {
    if (age < 0) {
        throwErr("bounds", "年龄不能为负数")
    }
    println("年龄有效:", age)
} (err) {
...
validateAge := (age) {
    if (age < 0 || age > 150) {
        throwErr("bounds", "年龄必须在 0-150 之间")
    }
    R> true
}

// 示例 3: 不在 try-catch 中使用（程序终止）
if (username == "") {
    throwErr("error", "用户名不能为空")  // 程序终止
}
```
